...
function () {
  ...
  _createClass(TextLayerBuilder, [{
    ...
    key: "_convertHighlightMatches",
    value: function _convertHighlightMatches(highlightMatches) {
      if (!highlightMatches) {
        return [];
      }
      ...
      var i = 0,
          iIndex = 0;
      var matches = Object.keys(highlightMatches).map(function (item) {
        return parseInt(item);
      });

      for (var m = 0, mm = matches.length; m < mm; m++) {
        var matchIdx = matches[m];
        var queryLen = highlightMatches[matchIdx].length;
    ...
    value: function _updateMatches() {
      ...
      if (findController.state.highlights) {
        var pageMatches = findController.pageHighlightMatches[pageIdx] || null;
        this.matches = this._convertHighlightMatches(pageMatches);
      } else {
        var _pageMatches = findController.pageMatches[pageIdx] || null;

        var pageMatchesLength = findController.pageMatchesLength[pageIdx] || null;
        this.matches = this._convertMatches(_pageMatches, pageMatchesLength);
      }
...
function () {
  ...
  _createClass(PDFFindController, [{
    ...
    key: "_calculateHighlightMatch",
    value: function _calculateHighlightMatch(highlights, pageIndex, pageContent) {
      var matches = [];
      var emOpeningTag = '<em>';
      var emClosingTag = '</em>';
      var that = this;
      ...
      highlights.forEach(function (highlight) {
        ...
        var _loop = function _loop() {
          ...
          emTags.forEach(function (emTag, index) {
            var phrase = emTag.replace(/<\/?em>/g, '');

            var phraseIdx = that._indexOfPhrase(pageContent, phrase, idx);
    ...
    value: function _nextMatch() {
      ...
      var highlights = this._state.highlights;
